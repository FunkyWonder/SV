...
#define FREEALL {free(xi);free(pnew);freemat(hessin,n);free(hdg);free(g);free(dg);}
...
if (z==NULL){printf("out of memory\n");exit(1);}
...
for (i=0;i<m;i++){
	...
	if (z[i]==NULL){printf("out of memory\n");exit(1);}
	}
return z;
}
...
for (i=0;i<n;i++) // the main loop over the columns to bereduced.
{
	...
	for (ll=0;ll<n;ll++) //Next, we reduce the rows...
	{
		if (ll != icol) //...except for the pivot one, of course.
		{
			dum=P[ll][icol];
			P[ll][icol]=0.0;
			for (l=0;l<n;l++) P[ll][l] -= P[icol][l]*dum;
		}
	}
...
if (indxr[l] != indxc[l]) for (k=0;k<n;k++)
{
	aux1=P[k][indxr[l]];
...
for (i=0;i<dim+1;i++) for (j=0;j<dim;j++) Pf[i][j]=x0[j];

for (i=0;i<dim;i++) Pf[i+1][i]=Pf[i+1][i]+lambda[i];

for (i=0;i<dim+1;i++) Pf[i][dim]=func(Pf[i],yaux,epsin,At,yni);
...
/*		printf("iter=%d\n",k);*/
	...
	else
	{
		...
		if (ftry<Pf[0][dim])
		{
			...
			if (ftry2<ftry)
			{
				for (j=0;j<dim;j++) Pf[dim][j]=Ptry2[j];
				Pf[dim][dim]=ftry2;
			}
			else
			{
				for (j=0;j<dim;j++) Pf[dim][j]=Ptry[j];
				Pf[dim][dim]=ftry;
			}
		}
		else
			...
			if (ftry>Pf[dim-1][dim])
			{
				...
				if (ftry>Pf[dim][dim])
				{
					for (j=1;j<dim+1;j++)
					{
						...
						prodcv(w,0.5,dim,z);
						for (j1=0;j1<dim;j1++) Pf[j][j1]=z[j1];
  ...
  /*  tol=2*(Pf[dim][dim]-Pf[0][dim])/(fabs(Pf[dim][dim])+fabs(Pf[0][dim])+EPS1); */
	tol=0.0;
	for (j=0;j<dim;j++) 	tol+=2.0*fabs(Pf[dim][j]-Pf[0][j])/(fabs(Pf[dim][j])+fabs(Pf[0][j])+EPS1);
}
...
for (;;) { /*Start of iteration loop.*/
	...
	*f=(*func)(x,y1,y2,Ai,z);
	if (alam < alamin) { /*Convergence on delta_x. For zero finding,the calling program should
		...
		*check=1;
		...
		if (alam == 1.0) tmplam = -slope/(2.0*(*f-fold-slope)); /*First time.*/
			...
			if (tmplam > 0.5*alam) tmplam=0.5*alam; /* lambda<=0.5*lamdbda_1.*/
		}
	}
	alam2=alam;
	f2 = *f;
...
if (h == 0.0) printf("h must be nonzero in dfridr.");
a=alocmat(10,10);
hh=h;
...
a=1.0;
for (i=0;i<k;i++) a=a*pdf_norm(delta[i],mu[i],sigma);
...
{
  if (hsV[*(int*)a] > hsV[*(int*)b]) return 1;
	  else if (hsV[*(int*)a] < hsV[*(int*)b]) return -1;
		else return 0;
}
...
{
  if (z<(-tcc)) return (tcc*log((z+tcc)/tcc+sqrt(1.0+pow((z+tcc)/tcc,2.0)))-tcc);
  else return z;
}
...
if (x<(-csqtr)) a=D1p/(ctrunc-1.0)*pow(fabs(x),1.0-ctrunc);
else {
	Ac=csqtr*pd1/(ctrunc-1.0);
	if (x<(csqtr)) a=Ac+PhiErf(x)-PhiErf(-csqtr);
	else a=2*Ac+2*PhiErf(csqtr)-1.0+D1p*pow(x,1.0-ctrunc)/(1.0-ctrunc);
}
...
for (cont=0;cont<B;cont++) hs[cont][1]=mu/(1.0-phi)+sqrt(sigma2/(1.0-phi2))*rnor();
...
for (t=i0;t<l;t++) {
	...
	val=0.0;
	for (cont=0;cont<B;cont++) val+=Probi[cont];
	...
	if (fabs(val)>0.0) {
		...
		for (cont=0;cont<B;cont++) Probi[cont]=Probi[cont]/val;
	
		piMalik[0]=Probi[0]/2;
		piMalik[B]=Probi[B-1]/2;
		for (cont=1;cont<B;cont++) piMalik[cont]=(Probi[cont]+Probi[cont-1])/2;
		...
		for (cont=0;cont<B;cont++) {
			if (sigmacount[cont]==0) hsV[cont]=hs[0][1];
			else {
				if (sigmacount[cont]==B) hsV[cont]=hs[B-1][1];
				else hsV[cont]=(hs[sigmacount[cont]][1]-hs[sigmacount[cont]-1][1])*hs[cont][0]+hs[sigmacount[cont]-1][1];
			}
		}
		
		for (cont=0;cont<B;cont++) hs[cont][1]=hsV[cont];
	}
	else {
		Like=-BIG;
		t=l+1;
...
for (i=0;i<(itno*trials);i++) {
	for (j=0;j<pdim;j++) tetainit[i][j]=muaux[j]*(0.99+0.02*uni());
}
...
minval=BIG;	
for (i=0;i<pdim;i++) tetaCons[i]=tetainit[0][i];
...
for (jcont=0;jcont<trials;jcont++) {
	...
	if (test[0][pdim]<minval) {
		for (i=0;i<pdim;i++) tetaCons[i]=test[0][i];
		minval=test[0][pdim];
	}
		
	fp=fopen(outputFile,"a");
	for (i=0;i<pdim;i++) fprintf(fp,"%.12f	",test[0][i]);
	fprintf(fp,"%.16f\n",test[0][pdim]);
	fclose(fp);
